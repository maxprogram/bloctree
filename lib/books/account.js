...
exports.add = function(account, category) {
    ...
    var existing = _.where(accounts, { name: account.name });
    if (existing.length) return Q.reject(new Error('Account "' +account.name+ '" already exists'));
    ...
    if (highestNum == -Infinity) highestNum = startNum - 1;
