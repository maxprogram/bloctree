 ...
 * @returns {promise:array} New entry
 */
exports.update = function(account, entry, details) {
    account = this.find(account);
    entry = _.where(account.entries, function(e) {
        return e[0] == entry;
    });
...
exports.transaction = function(drAccount, crAccount, details) {
    ...
    return Q.all([
        this.record.entry(drAccount, drEntry),
        this.record.entry(crAccount, crEntry)
    ]);
};
